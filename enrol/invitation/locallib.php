    ...
    public function send_invitations($data, $resend = false) {
        ...
        if (has_capability('enrol/invitation:enrol', context_course::instance($data->courseid))) {
            ...
            if (!empty($data->email)) {
                ...
                $messagehtml = get_string('emailmsgtxt', 'enrol_invitation', $message_params);
                ...
                if (empty($invitation->show_from_email)) {
                    $fromuser = enrol_invitation_generate_email_user($CFG->supportemail, $SITE->fullname);
                }

                // Send invitation to the user.
                $contactuser = enrol_invitation_generate_email_user($invitation->email);
                email_to_user($contactuser, $fromuser, $invitation->subject, $messagehtml, $messagetext, '', '', true);
 ...
 */
function enrol_invitation_generate_email_user($email, $name='') {
